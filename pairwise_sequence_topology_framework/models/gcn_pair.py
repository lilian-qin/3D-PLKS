...
class GCNNet(torch.nn.Module):
    def __init__(self, n_output=1, n_filters=32, embed_dim=128,
                 num_features_xd=78, num_features_xt=25, output_dim=128,max_seq_len=108,
                 dropout=0.2, pool_method='diff'):
        ...
        # ===== Comparison + MLP head =====
        # h_A and h_B are each [B, 2*output_dim] = [B, 256]
        pair_dim = 2 * output_dim  # 256

        if pool_method == 'diff':
            mlp_input_dim = pair_dim
        elif pool_method == 'diff_lig':
            mlp_input_dim = pair_dim  # [ligand, prot_A - prot_B]
    ...
    def encode_ligand(self, x, edge_index, batch):
        """Encode ligand graph through GCN layers. Called once."""
    ...
    def forward(self, data):
        ...
        if self.pool_method == 'diff':
            ...
            h = h_A - h_B  # ligand cancels out
        elif self.pool_method == 'diff_lig':
            h = torch.cat((x_lig, xt_A - xt_B), dim=1)  # keep ligand
