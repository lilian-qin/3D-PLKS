...
class Typer:
    ...
    def get_types(
        ...
        if return_occupancy_value:
            ...
            # Build numpy arrays for fast coordinate matching
            pdb_coords = pdb_df[["x_coord", "y_coord", "z_coord"]].values
            pdb_occupancies = pdb_df["occupancy"].values

            for i in range(len(df)):
                row = df.iloc[i]
                query = np.array([row["x"], row["y"], row["z"]])

                # Find closest atom by coordinate distance (handles float precision)
                dists = np.linalg.norm(pdb_coords - query, axis=1)
                min_idx = np.argmin(dists)

                if dists[min_idx] < 0.01:  # within 0.01 Angstrom tolerance
                    occupancy = int(pdb_occupancies[min_idx])
                else:
                    occupancy = 1  # default fallback
